        ...
        inputs: [
          {
            ...
            helper: "ENS supported",
          },
          { label: "Amount", type: "amount", placeholder: "100.00", helper: "USDC" },
        ],
        previewTemplate: (v, c) => `Send **${v[1] || "0"} ${c}** to **${v[0] || "..."}**`,
        mockResult: "100.00 USDC sent",
      },
      {
        id: "approve",
        icon: ShieldCheck,
        ...
        category: "action",
        inputs: [
          { label: "Spender", type: "address", placeholder: "Uniswap Router or 0x..." },
          { label: "Limit", type: "amount", placeholder: "1000.00", helper: "USDC" },
        ],
        previewTemplate: (v, c) =>
          `Allow **${v[0] || "..."}** to spend up to **${v[1] || "0"} ${c}**`,
        mockResult: "Approval set",
      },
      {
        id: "balance",
        icon: Eye,
    ...
    actions: [
      {
        ...
        previewTemplate: (v) => `Wrap **${v[0] || "0"} ETH** into WETH`,
        mockResult: "1.0 WETH received",
      },
      {
        id: "withdraw",
        icon: Send,
        ...
        previewTemplate: (v) => `Unwrap **${v[0] || "0"} WETH** into ETH`,
        mockResult: "1.0 ETH received",
      },
      {
        id: "balance",
        icon: Eye,
        label: "Check Balance",
        description: "View WETH balance of any wallet",
        category: "view",
        inputs: [
          {
            label: "Wallet to check",
        ...
        previewTemplate: (v) => `Check WETH balance of **${v[0] || "your wallet"}**`,
        mockResult: "4.2100 WETH",
      },
    ],
  },
];
...
export function ContractInteractionPreview() {
  ...
  const selectContract = (c: ContractPreset) => {
    setContract(c);
    setStep("actions");
  };
  const selectAction = (a: Action) => {
    setSelectedAction(a);
    setInputValues(a.inputs.map(() => ""));
    setResult("");
    setStep(a.inputs.length > 0 ? "form" : "preview");
  };
  const goBack = () => {
    if (step === "form") setStep("actions");
    else if (step === "preview") setStep(selectedAction?.inputs.length ? "form" : "actions");
    ...
    <div style={{ ...previewCard, maxWidth: 420, width: "100%", margin: "0 auto" }}>
      ...
      <div style={{ ...previewHeader }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {contract ? (
            <img
              src={cryptoLogo(contract.ticker)}
              alt=""
              width={20}
              height={20}
          ...
          {PRESETS.map((p) => (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <span style={{ fontSize: 13, color: "var(--w3-gray-600)", fontFamily: monoFont }}>
          ...
          <div style={{ margin: "8px 14px 0" }}>
            ...
            <div
              style={{
                padding: "12px",
                borderRadius: 10,
                border: "1px dashed var(--w3-border-subtle)",
                display: "flex",
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <span style={{ fontSize: 13, color: "var(--w3-gray-600)" }}>
                          {action.description}
                        </span>
                      </div>
                      <ArrowRight size={14} style={{ color: "var(--w3-gray-400)" }} />
              ...
              <div key={idx}>
                ...
                <input
                  ...
                  style={{
                    ...
                    background: "transparent",
                    fontSize: 14,
                    fontFamily: input.type === "address" ? monoFont : "inherit",
                    color: "var(--w3-gray-900)",
                    outline: "none",
                  }}
                />
      ...
      {step === "preview" && selectedAction && contract && (
        <div style={{ padding: 20 }}>
          ...
          >
            ...
            >
              {selectedAction
                .previewTemplate(inputValues, contract.name)
                .split("**")
                .map((part, i) =>
                  i % 2 === 1 ? (
                    <strong key={i} style={{ color: "var(--w3-accent)" }}>
                      {part}
                    </strong>
                  ) : (
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              onClick={() => setStep(selectedAction.inputs.length ? "form" : "actions")}
              style={{
                flex: 1,
                padding: 12,
                borderRadius: 10,
            ...
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  selectedAction.category === "view" ? "var(--w3-accent)" : "var(--w3-gray-900)",
                color: "#fff",
                fontSize: 14,
                fontWeight: 600,
                cursor: "pointer",
            ...
            <>
              ...
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 15, fontWeight: 500, color: "var(--w3-gray-900)" }}>
                  {selectedAction.category === "view"
                    ? "Querying..."
                    : "Waiting for confirmation..."}
                </div>
                <div style={{ fontSize: 13, color: "var(--w3-gray-500)", marginTop: 4 }}>
                  {selectedAction.category === "view" ? "Reading contract" : "Sign in your wallet"}
                </div>
              </div>
            </>
          ) : (
            <>
              ...
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 15, fontWeight: 600, color: "var(--w3-gray-900)" }}>
                  {selectedAction.category === "view" ? "Result" : "Confirmed"}
                </div>
                <div
                  style={{
                    fontSize: 16,
      ...
      >
        <span style={{ fontSize: 13, color: "var(--w3-gray-500)" }}>
          {contract ? `${contract.actions.length} actions` : `${PRESETS.length} contracts`}
        </span>
