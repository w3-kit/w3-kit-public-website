...
export function NFTMarketplacePreview() {
  ...
  const bestId =
    filtered.length > 0
      ? filtered.reduce((b, l) => (parseFloat(l.price) < parseFloat(b.price) ? l : b), filtered[0])
          .id
      : null;
    ...
    <div style={{ ...previewCard, maxWidth: 400, width: "100%", margin: "0 auto" }}>
        ...
        {(["all", "OpenSea", "Blur", "LooksRare"] as Marketplace[]).map((m) => (
          ...
          >
            {m !== "all" && (
              <img
                src={domainLogo(
                  m === "OpenSea" ? "opensea.io" : m === "Blur" ? "blur.io" : "looksrare.org",
                  14,
                )}
                alt=""
                width={12}
                height={12}
                style={{ borderRadius: 3, opacity: market === m ? 1 : 0.7 }}
              />
            )}
            {m === "all" ? "All" : m}
          </button>
      ...
      <div style={{ padding: "12px 20px", display: "flex", flexDirection: "column", gap: 4 }}>
        {filtered.map((listing) => {
          ...
          const savings = listing.lastSale
            ? ((parseFloat(listing.lastSale) - parseFloat(listing.price)) /
                parseFloat(listing.lastSale)) *
              100
            : null;
            ...
            >
              ...
              >
                <img
                  ...
                  style={{
                    ...
                    height: "100%",
                    objectFit: "cover",
                    opacity: loaded.has(listing.id) ? 1 : 0,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
                  ...
                  >
                    <img
                      src={domainLogo(listing.domain, 14)}
                      alt=""
                      width={12}
                      height={12}
                      style={{ borderRadius: 3 }}
                    />
                    <span style={{ fontSize: 10, fontWeight: 500, color: "var(--w3-accent)" }}>
                      {listing.marketplace}
                    </span>
                  </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                <div style={{ textAlign: "right" }}>
                  ...
                  {listing.lastSale && (
                    <span
                      style={{ fontSize: 11, color: "var(--w3-gray-500)", fontFamily: monoFont }}
                    >
